...
/* 从cnf文件读取到并建立数据结构 */
void CNF_EXACTER(CNF& cnf,char [],int&);
...
int ChooseBetterValue(const CNF& cnf, int);
Status DPLL(CNF& cnf, Stack& history,int);
...
void eatline(void);
void trans_to_sodu(const CNF& cnf,FILE*,Hanidoku);
void read_doku(Hanidoku*& sets, const char filename[], int size);
void print_doku(const Hanidoku&);
